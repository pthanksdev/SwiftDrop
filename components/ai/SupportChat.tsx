...
import { MessageSquare, X, Send, Sparkles, Minus, Maximize2, User, Bot, ExternalLink, Globe, Loader2 } from 'lucide-react';
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="fixed bottom-8 right-8 w-16 h-16 bg-[#845C00] text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-[100] group"
      >
    ...
    <div className={cn(
      ...
      <div className="p-6 bg-slate-900 text-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
           <div className="w-10 h-10 bg-[#845C00] rounded-xl flex items-center justify-center shadow-lg"><Sparkles size={20} /></div>
           <h4 className="font-black text-sm">SwiftDrop AI</h4>
        </div>
        <div className="flex gap-1">
           <button onClick={() => setIsMinimized(!isMinimized)} className="p-2 hover:bg-white/10 rounded-lg transition-colors"><Minus size={16} /></button>
      ...
      {!isMinimized && (
        <>
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 scroll-smooth">
            {history.map((msg, i) => (
              <div key={i} className={cn("flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300", msg.role === 'user' ? "items-end" : "items-start")}>
                <div className={cn("flex gap-3", msg.role === 'user' ? "flex-row-reverse" : "")}>
                  <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 shadow-sm", msg.role === 'user' ? "bg-slate-900 text-white" : "bg-[#845C00] text-white")}>
                ...
                {msg.grounding && msg.grounding.length > 0 && (
                  <div className="ml-11 flex flex-wrap gap-2">
                    {msg.grounding.map((chunk, j) => chunk.web && (
                      <a key={j} href={chunk.web.uri} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-bold text-[#845C00] hover:border-[#845C00] transition-colors">
          ...
          <form onSubmit={handleSend} className="p-6 bg-white border-t border-slate-100 shrink-0">
            <div className="relative">
              <input 
                ...
                className="w-full bg-slate-50 border-none rounded-2xl py-4 pl-4 pr-12 text-sm outline-none focus:ring-2 focus:ring-[#845C00]/10 transition-all placeholder:text-slate-400 font-medium" 
              />
              <button 
                type="submit" 
                disabled={!message.trim() || loading}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-[#845C00] text-white rounded-xl flex items-center justify-center shadow-lg disabled:opacity-50 transition-all active:scale-95"
              >
                <Send size={16} />
              </button>
            </div>
